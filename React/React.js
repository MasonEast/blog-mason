...
(function (global, factory) {
    ...
    function extend (obj, props) {
        for (var i in props) {
            if (hasOwnProperty.call(props, i)) {        //使用call调用而不是直接调用该方法，一方面是防止方法被重写，无法执行，另一方面可以使它在理论上更快，因为[[Prototype]]链不必被遵循
                obj[i] = props[i];
            }
        }
        return obj;
    ...
    function Component (props, context) {       //Component构造函数作为React的父类，具有updater更新机制， setState， forceUpdate等方法便于React继承。
    ...
    var boundaries = Renderer.boundaries;
    var batchedtasks = [];
    function render (vnode, root, callback) {               //这就是reactDOM.render()执行的方法， 接受三个参数， 组件babel编译后的虚拟DOM， 真实DOM， 回调函数
        debugger
    ...
    function createContainer (root, onlyGet, validate) {
        ...
        if (!validate(root)) {                                  //判断root是不是真实的DOM节点
        ...
        if (useProp) {
            root.anuProp = void 0;
            if (get(root)) {                                    //判断是不是已经存在fiber实例，存在就返回已创建的fiber
                return get(root);
            }
        } else {
        ...
        var container = new Fiber({                         //第一次render会走到这里，创建一个Fiber实例，并返回。
            stateNode: root,
            tag: 5,
            name: 'hostRoot',
            contextStack: [{}],
    ...
    var DOMRenderer = createRenderer({                      //创建一个Renderer对象，将传入的参数加到该对象上去
        render: render,
    ...
    var React = void 0;                             //替代undefined， 防止被重写
    if (prevReact && prevReact.eventSystem) {       //判断宿主环境中是否有react，有的话用宿主环境的，否则用anu
        React = prevReact;
    } else {
        var render$1 = DOMRenderer.render,          //reactDOM.render()方法
